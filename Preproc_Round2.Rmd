...
Stat_Day_1 <- unique(Six_Days$LOCATION[Six_Days$DOY_Days == 328])
...
All_Stat[!(All_Stat %in% Stat_DOY[["9"]])] # Day 9
All_Stat[!(All_Stat %in% Stat_DOY[[2]])] 
All_Stat[!(All_Stat %in% Stat_DOY[[3]])]
All_Stat[!(All_Stat %in% Stat_DOY[[4]])]
All_Stat[!(All_Stat %in% Stat_DOY[[5]])]
All_Stat[!(All_Stat %in% Stat_DOY[[6]])]
...
test <- subset(Day_One, Day_One$LOCATION == 1117)
...
# Test which stations are in ALL DOY subsets 
...
# Test if stations REALLY are not occuring AT ALL on a given day
Missing_DOY[[1]]
Day_One <- subset(Six_Days, Six_Days$DOY_Days == 9)
any(Day_One$LOCATION == 1117)
...
test <- as.data.frame(lapply(Missing_DOY, length))
colnames(test) <- unique(Six_Days$DOY_Days)
library(reshape2)
test <- melt(test)
colnames(test) <- c("DOY", "Missing")

library(ggplot2)
ggplot(data= test, aes(x= DOY, y= Missing)) + geom_bar(stat="identity") + ylab("Number of missing stations")

barplot(test$Missing, names.arg = test$DOY, xlab = "DOY", ylab = "Number of missing stations")




##### Now test how many unique stations there are in test/train per DOY
#(Because 'suspect' data/NA values have now been removed + these are smaller subsets)
...
# Somehow the test and train sets only contain five days worth of data? Day 68 is missing !!!
