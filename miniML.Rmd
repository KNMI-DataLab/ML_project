...
new_data <- cbind(Train_data[ ,5:6], Train_data[8:10])
...
CenterScale <- preProcess(new_data)
...
# Build the mlp model
MLPModel <-  train(x  = Train_data[7:(length(colnames(data_GMS)))], y = Train_data$TEMP, method = "mlp")
...
MLP_test <- mlp(x  = Train_data[7:(length(colnames(data_GMS)))], y = Train_data$TEMP)
plot.nnet(MLP_test)
...
plotActMap(Train_data[7:(length(colnames(data_GMS)))])

# Plot the iterative error
plotIterativeError(MLPModel$finalModel)

# Extract predictions for MLP
MLP_Predict <- extractPrediction(list(MLPModel), testX = Test_data[7:(length(colnames(data_GMS)))], testY = Test_data$TEMP)
...
ggplot(data = MLP_Predict) + geom_point(aes(x = obs, y = pred))
...
RBFModel2 <- rbf(x = Train_data[7:(length(colnames(data_GMS)))], y = Train_data$TEMP)

# High RMSE
plot(RBFModel)
head(summary(RBFModel))

# Model 2 has high SSE
plotIterativeError(RBFModel2)

# Model from train doesn't predict (pred = 0)
RBF_Predict <- extractPrediction(list(RBFModel), testX = Test_data[7:(length(colnames(data_GMS)))], testY = Test_data$TEMP)

# predicting for the other model:
RBF_Predict2 <- predict(RBFModel2, Test_data[7:(length(colnames(data_GMS)))])

# For the other model, observed vs. predicted:
ggplot() + geom_point(aes(x = Test_data$TEMP, y = RBF_Predict2))
...
```{r}
# Build the SVM model
SVMradModel <- train(x  = Train_data[7:(length(colnames(data_GMS)))], y = Train_data$TEMP, method = "svmRadialCost", scale = FALSE)
compiler::setCompilerOptions(optimize = 1)
...
SVMrad_Predict <- extractPrediction(list(SVMradModel), testX = Test_data[7:(length(colnames(data_GMS)))], testY = Test_data$TEMP)
