...
Tree_Predict2 <- extractPrediction(list(TreeModel), testX = Test_data[7:(length(colnames(data_GMS)))])
...
# It is also possible to use the randomforest package directly. This method can also use the two different ways
library(parallelRandomForest)

system.time(RFModel3 <- randomForest(x = Train_data[7:(length(colnames(data_GMS)))], y = Train_data$TEMP))
...
RF_Predict <-  extractPrediction(list(RFModel), testX = Test_data[7:(length(colnames(data_GMS)))])
